...
from database.supabase_client import save_report, get_reports, delete_report
...
<div style="margin-bottom:2rem;">
    <div style="font-size:0.68rem; font-weight:500; letter-spacing:3px; text-transform:uppercase;
                color:rgba(26,60,35,0.35); margin-bottom:0.6rem; font-family:'Inter',sans-serif;">Reports</div>
...
if result and role:
    ...
    with col_name:
        report_name = st.text_input("Report name", value=f"{role} Analysis",
...
if not reports:
    ...
    for i, report in enumerate(reports):
        ...
        c1, c2, c3, c4, c5 = st.columns([2.5, 1, 1, 0.8, 0.8], gap="medium")
        ...
        with c4:
            ...
            if st.button("View →", key=f"view_{report['id']}", use_container_width=True, type="primary"):
                # Restore this report into session state and go to Dashboard
                st.session_state.result = {
                    "score":      report["score"],
                    "matched":    report.get("matched_skills", []),
                    "missing":    report.get("missing_skills", []),
                    "readiness":  report.get("readiness", ""),
                    "salary":     report.get("salary", "N/A"),
                    "difficulty": report.get("difficulty", "N/A"),
                }
                st.session_state.role        = report["role"]
                st.session_state.user_skills = report.get("matched_skills", [])
                st.session_state.loaded_from_report = report["report_name"]
